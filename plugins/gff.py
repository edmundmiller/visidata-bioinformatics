...
# Define and register options properly
@VisiData.api
def init_gff(vd):
    vd.option(
        "gff_to_bed_name_attr",
        "Name",
        "GFF attribute to use as name when converting to BED",
    )
    vd.option(
        "gff_to_bed_score_attr",
        "score",
        "GFF attribute to use as score when converting to BED",
    )
    vd.option(
        "gff_default_score", "0", "Default score to use when converting GFF to BED"
    )
...
class GffSheet(Sheet):
    ...
    def openRow(self, row):
        return AttributesSheet(
            name=f"attributes_{row[0]}_{row[3]}_{row[4]}", source=row[8]
        )
    ...
    columns = [
        ...
        Column(
            name="start",
            getter=lambda c, r: int(r[3]) if r[3] != "." else None,
            type=int,
        ),
        Column(
            name="end", getter=lambda c, r: int(r[4]) if r[4] != "." else None, type=int
        ),
        Column(
            name="score",
            getter=lambda c, r: float(r[5]) if r[5] != "." else None,
            type=float,
        ),
        Column(
            name="strand", getter=lambda c, r: r[6] if r[6] != "." else None, type=str
        ),
        Column(
            name="phase", getter=lambda c, r: r[7] if r[7] != "." else None, type=str
        ),
        Column(
            name="attributes",
            getter=lambda c, r: r[8] if r[8] != "." else None,
            type=str,
        ),
    ]

    # Remove @asyncthread from iterload
    def iterload(self):
            ...
            for line in Progress(lines, "loading"):
                try:
                    ...
                    if not fields or len(fields) < 9:
                        vd.warning(
                            f"skipping invalid line (expected 9 fields, got {len(fields)}): {line}"
                        )
                        continue
